...
const createFlashCard = expressAsyncHandler(async (req:any, res) => {
  ...
  const flashCard = await FlashCard.create({ user:req.user._id, question, answer, language, tutor });
  res.status(201).json({
    status: "success",
    flashCard,
  });
...
const getFlashCardByLanguage = expressAsyncHandler(async (req:any, res) => {
    const flashCards = await FlashCard.find({ user:req.user._id,language: req.params.language });
    res.status(200).json({
        status: "success",
        flashCards,
    });
...
const getFlashCardById = expressAsyncHandler(async (req:any, res) => {
    const flashCard = await FlashCard.findOne({ user:req.user._id, 
...
export { createFlashCard, getAllFlashCards, getFlashCardByLanguage, getFlashCardByTutor, getFlashCardById, updateFlashCard, deleteFlashCard };